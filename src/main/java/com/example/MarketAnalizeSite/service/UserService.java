...
import com.example.MarketAnalizeSite.models.users.Roles;
...
public class UserService {
    ...
    public void addAdminRole(Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new UsernameNotFoundException(STR."Пользователь: \{userId} не найден"));
    ...
    public void toggleAdminRole(Long userId) {
        ...
        if (user.getRoles().contains(Roles.ROLE_ADMIN)) {
            user.getRoles().remove(Roles.ROLE_ADMIN);
            log.info("Роль администратора отключена у пользователя с id: {}", userId);
        } else {
            user.getRoles().add(Roles.ROLE_ADMIN);
            log.info("Роль администратора включена пользователю с id: {}", userId);
        }
        userRepository.save(user);
    }
    ...
    public User updateUser(Long id, User user) {
        ...
        User existingUser = userRepository.findById(id).orElseThrow(() ->
                new EntityNotFoundException(STR."Пользователь с id: \{id} не найден"));
        ...
        try {
            ...
            throw new PersistenceException(STR."Не удалось обновить пользователя с id: \{id}", e);
        }
    }
